...
const Cart = () => {
    ...
    return (
        <Fragment>
            {
                productData.length>0?(<div className='mt-5 flex flex-col'>
                                    ...
                                    <tbody key={item._id}>
                                        <tr className='bg-white border-b-[1px] border-b-zinc-300'>
                                            <th scope='row' className='px-6 py-4 flex items-center gap-3'>
                                                <X onClick={()=>{dispatch(deleteProduct(item?._id)), toast.success(`${item.title} deleted Successfully`)}} className='w-4 h-4 hover:text-red-600 cursor-pointer duration-200' />
                                                <Image src={item?.image} alt='none' className='w-24 object-contain' width={500} height={500} />
                                            ...
                                            <td className='px-6 py-4 flex items-center gap-4'>
                                                <span className='border border-zinc-300 p-1 rounded-md hover:border-zinc-800 cursor-pointer duration-200 inline-flex items-center justify-center'>
                                                    <Minus onClick={()=>item?.quantity>1?(dispatch(decreaseQuantity(item)),toast.success(`${item.title} delete Successfully`)):(dispatch(deleteProduct(item._id)),toast.success(`${item.title} deleted Successfully`))} className='w-4 h-4'/>
                                                </span>
                                                <span>{item?.quantity}</span>
                                                <span className='border border-zinc-300 p-1 rounded-md hover:border-zinc-800 cursor-pointer duration-200 inline-flex items-center justify-center'>
                                                    <Plus onClick={()=>{dispatch(increaseQuantity(item)),toast.success(`${item.title} increased Successfully`)}} className='w-4 h-4'/>
                                                </span>
                                            </td>
                                            <td className='px-6 py-4'>
                                                <FormatedPrice price={item?.price*item?.quantity} />
                                            </td>
                    ...
                    <button onClick={handleReset} className='bg-zinc-950 text-zinc-200 w-36 py-3 mt-5 rounded-md uppercase text-xs font-semibold hover:bg-red-700 hover:text-white duration-200'>
                        Reset Cart
                    </button>
                    <div className="mt-4 bg-white max-w-xl p-4 flex flex-col gap-1">
                        ...
                        <p className='flex items-center justify-between'>
                            Price <span><FormatedPrice price={rowPrice} /></span>
                        </p>
                        <p className='flex items-center justify-between'>
                            Discount <span><FormatedPrice price={rowPrice-totalAmt} /></span>
                        </p>
                        <p className='flex items-center justify-between'>
                            Discount <span><FormatedPrice className='font-semibold text-lg' price={totalAmt} /></span>
                        </p>
                    ...
                    <div className='py-10 flex flex-col gap-1 items-center justify-center'>
                        ...
                        <Link href={'/'} className='text-sm uppercase font-semibold underline underline-offset-2 hover:text-designColor duration-200 cursor-pointer'>Go Back to Shopping</Link>
                    </div>
                )   
            }
